    ...
    <div>
      <div className="adm-section-hdr">
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <div className="adm-search-bar"><span style={{ color: "var(--adm-t3,#8aa0cc)" }}>🔍</span><input placeholder="Search staff…" value={search} onChange={e => setSearch(e.target.value)} /></div>
      ...
      <div className="adm-card">
        <table className="adm-table">
          ...
          <tbody>
            {visible.length === 0
              ? <tr><td colSpan={6} style={{ textAlign: "center", padding: 30, color: "var(--adm-t3,#8aa0cc)" }}>No staff found</td></tr>
              : visible.map(u => (
                <tr key={u.uid}>
                  <td>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <span style={{ color: "var(--adm-t1,#0a1628)", fontWeight: 900 }}>{u.name}</span>
                    </div>
                  </td>
                  <td style={{ color: "var(--adm-t2,#4a6699)" }}>{u.email}</td>
                  <td>
                  ...
                  <td>{u.ward || <span style={{ color: "var(--adm-t3,#c8d8f8)" }}>—</span>}</td>
                  <td style={{ color: "var(--adm-t2,#4a6699)" }}>{u.createdAt?.toDate ? u.createdAt.toDate().toLocaleDateString() : "—"}</td>
    ...
    <div>
      <div className="adm-section-hdr">
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <div className="adm-search-bar"><span style={{ color: "var(--adm-t3,#8aa0cc)" }}>🔍</span><input placeholder="Search patients…" value={search} onChange={e => setSearch(e.target.value)} /></div>
        </div>
      </div>
      <div className="adm-card">
        <table className="adm-table">
          ...
          <tbody>
            {visible.length === 0
              ? <tr><td colSpan={7} style={{ textAlign: "center", padding: 30, color: "var(--adm-t3,#8aa0cc)" }}>No records found</td></tr>
              : visible.map(p => (
                <tr key={p.id}>
                  <td style={{ color: "var(--adm-t1,#0a1628)", fontWeight: 900 }}>{p.name}</td>
                  <td><span style={{ fontFamily: "monospace", color: "var(--adm-t1,#0d2b6b)", fontWeight: 900 }}>{p.emr || "—"}</span></td>
                  <td>{p.ward?.split("–")[0]?.trim() || "—"}</td>
                  <td>{p.diagnosis || <span style={{ color: "var(--adm-t3,#c8d8f8)" }}>—</span>}</td>
                  <td><span className={`adm-badge adm-badge-${p.status === "discharged" ? "amber" : "green"}`}>{p.status || "active"}</span></td>
                  <td style={{ color: "var(--adm-t2,#4a6699)" }}>{p.admission || "—"}</td>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        <div className="adm-card">
          ...
          <div className="adm-card-body">
            ...
            <div style={{ fontSize: 13, fontWeight: 700, lineHeight: 2.2, color: "var(--adm-t2,#1a2e5a)" }}>
              <div>Email: <span style={{ color: "var(--adm-t1,#0d2b6b)", fontWeight: 900 }}>{ADMIN_EMAIL}</span></div>
              <div>Role: <span className="adm-badge adm-badge-gold">System Administrator</span></div>
              <div>Access Level: <span style={{ fontWeight: 900, color: "var(--adm-t1,#0d2b6b)" }}>Full System Control</span></div>
            </div>
          </div>
        </div>
        <div className="adm-card">
          ...
          <div className="adm-card-body">
            <div style={{ fontSize: 13, fontWeight: 700, lineHeight: 2.2, color: "var(--adm-t2,#1a2e5a)" }}>
              ...
              <div>AI Engine: <span style={{ fontWeight: 900, color: "var(--adm-t1,#0d2b6b)" }}>Claude (Anthropic)</span></div>
            </div>
          </div>
        </div>
        <div className="adm-card">
