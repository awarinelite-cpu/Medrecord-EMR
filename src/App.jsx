...
const FB = {
  ...
  deleteAnnouncement: async (id) => {
    await setDoc(doc(db, "announcements", id), { deleted: true }, { merge: true });
  },
  saveSystemLog: async (action, detail, adminEmail) => {
    const id = "LOG-" + Math.random().toString(36).slice(2,10);
    await setDoc(doc(db, "systemLogs", id), { id, action, detail, adminEmail, timestamp: serverTimestamp() });
  },
  onSystemLogs: (cb) => {
  ...
  deletePatient: async (id) => {
    await setDoc(doc(db, "patients", id), { deleted: true, deletedAt: serverTimestamp() }, { merge: true });
  },
};
...
function LoginPage({ onLogin }) {
  ...
  const doLogin = async () => {
    ...
    // Admin shortcut — bypass Firebase auth
...
.admin-root{font-family:'Inter',sans-serif;background:#0a0a0f;color:#e2e8f0;min-height:100vh;display:flex}
.admin-root *{box-sizing:border-box;margin:0;padding:0}
.admin-root ::-webkit-scrollbar{width:5px}
.admin-root ::-webkit-scrollbar-track{background:transparent}
.admin-root ::-webkit-scrollbar-thumb{background:#2d2d3d;border-radius:3px}
.adm-sidebar{width:230px;min-height:100vh;background:#0d0d15;border-right:1px solid #1e1e2e;display:flex;flex-direction:column;position:fixed;top:0;left:0;bottom:0;z-index:100;overflow-y:auto}
.adm-main{flex:1;margin-left:230px;display:flex;flex-direction:column;min-height:100vh;background:#0a0a0f}
.adm-logo{padding:20px 16px;border-bottom:1px solid #1e1e2e}
.adm-logo-badge{display:inline-flex;align-items:center;gap:10px}
.adm-logo-icon{width:36px;height:36px;background:linear-gradient(135deg,#7c3aed,#a855f7);border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0}
.adm-logo-text{font-size:15px;font-weight:800;color:#fff;letter-spacing:-.3px}
.adm-logo-sub{font-size:9px;color:#6b7280;text-transform:uppercase;letter-spacing:1px;margin-top:1px}
.adm-nav{flex:1;padding:10px 8px}
.adm-nav-section{font-size:9px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:1.2px;padding:12px 8px 4px}
.adm-nav-btn{display:flex;align-items:center;gap:9px;width:100%;padding:8px 10px;border:none;border-radius:8px;background:none;color:#6b7280;font-size:12px;font-weight:500;cursor:pointer;transition:all .15s;margin-bottom:1px;text-align:left;font-family:'Inter',sans-serif}
.adm-nav-btn:hover{background:#1a1a2e;color:#e2e8f0}
.adm-nav-btn.active{background:linear-gradient(135deg,rgba(124,58,237,.2),rgba(168,85,247,.1));color:#a855f7;border:1px solid rgba(168,85,247,.25)}
.adm-nav-btn .adm-ni{font-size:15px;width:18px;text-align:center;flex-shrink:0}
.adm-nav-badge{margin-left:auto;background:#7c3aed;color:#fff;font-size:9px;font-weight:700;padding:1px 6px;border-radius:10px}
.adm-user-area{padding:12px 12px;border-top:1px solid #1e1e2e;display:flex;align-items:center;gap:9px}
.adm-user-avatar{width:32px;height:32px;border-radius:50%;background:linear-gradient(135deg,#7c3aed,#a855f7);display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:800;flex-shrink:0;color:#fff}
.adm-user-name{font-size:11px;font-weight:700;color:#e2e8f0}
.adm-user-role{font-size:10px;color:#7c3aed;font-weight:600}
.adm-topbar{height:56px;background:#0d0d15;border-bottom:1px solid #1e1e2e;display:flex;align-items:center;justify-content:space-between;padding:0 24px;position:sticky;top:0;z-index:50}
.adm-topbar-title{font-size:17px;font-weight:800;color:#fff;letter-spacing:-.3px}
.adm-topbar-sub{font-size:11px;color:#6b7280;margin-top:1px}
...
.adm-stat{background:#0d0d15;border:1px solid #1e1e2e;border-radius:14px;padding:18px 20px;position:relative;overflow:hidden}
.adm-stat-accent{position:absolute;top:0;left:0;right:0;height:3px;border-radius:14px 14px 0 0}
.adm-stat-icon{font-size:22px;margin-bottom:8px}
.adm-stat-val{font-size:28px;font-weight:800;color:#fff;font-family:'Inter',sans-serif;letter-spacing:-1px}
.adm-stat-label{font-size:11px;color:#6b7280;font-weight:500;margin-top:2px}
.adm-stat-delta{font-size:10px;font-weight:600;margin-top:6px;display:inline-flex;align-items:center;gap:3px;padding:2px 7px;border-radius:20px}
.adm-card{background:#0d0d15;border:1px solid #1e1e2e;border-radius:14px;overflow:hidden;margin-bottom:20px}
.adm-card-header{padding:14px 18px;border-bottom:1px solid #1e1e2e;display:flex;align-items:center;justify-content:space-between}
.adm-card-title{font-size:13px;font-weight:700;color:#e2e8f0}
.adm-card-body{padding:18px}
.adm-table{width:100%;border-collapse:collapse}
.adm-table th{font-size:10px;font-weight:700;color:#4b5563;text-transform:uppercase;letter-spacing:.8px;padding:8px 14px;border-bottom:1px solid #1e1e2e;text-align:left}
.adm-table td{padding:10px 14px;border-bottom:1px solid #0f0f1a;font-size:12px;color:#9ca3af;vertical-align:middle}
.adm-table tr:last-child td{border-bottom:none}
.adm-table tr:hover td{background:#111120;color:#e2e8f0}
.adm-badge{display:inline-flex;align-items:center;padding:2px 8px;border-radius:20px;font-size:10px;font-weight:700;letter-spacing:.3px}
.adm-badge-green{background:rgba(16,185,129,.1);color:#10b981;border:1px solid rgba(16,185,129,.2)}
.adm-badge-red{background:rgba(239,68,68,.1);color:#ef4444;border:1px solid rgba(239,68,68,.2)}
.adm-badge-yellow{background:rgba(245,158,11,.1);color:#f59e0b;border:1px solid rgba(245,158,11,.2)}
.adm-badge-purple{background:rgba(168,85,247,.1);color:#a855f7;border:1px solid rgba(168,85,247,.2)}
.adm-badge-blue{background:rgba(59,130,246,.1);color:#3b82f6;border:1px solid rgba(59,130,246,.2)}
.adm-btn{display:inline-flex;align-items:center;gap:5px;padding:7px 14px;border-radius:8px;border:none;font-size:12px;font-weight:600;cursor:pointer;transition:all .15s;font-family:'Inter',sans-serif;white-space:nowrap}
.adm-btn-primary{background:linear-gradient(135deg,#7c3aed,#a855f7);color:#fff}
.adm-btn-primary:hover{opacity:.9}
.adm-btn-danger{background:rgba(239,68,68,.1);color:#ef4444;border:1px solid rgba(239,68,68,.2)}
.adm-btn-danger:hover{background:rgba(239,68,68,.2)}
.adm-btn-ghost{background:#1a1a2e;color:#9ca3af;border:1px solid #1e1e2e}
.adm-btn-ghost:hover{color:#e2e8f0;background:#252538}
.adm-btn-sm{padding:4px 10px;font-size:11px}
.adm-btn:disabled{opacity:.5;cursor:wait}
.adm-input{width:100%;padding:8px 12px;background:#111120;border:1px solid #1e1e2e;border-radius:8px;color:#e2e8f0;font-size:13px;outline:none;font-family:'Inter',sans-serif;transition:border-color .15s}
.adm-input:focus{border-color:#7c3aed}
.adm-input::placeholder{color:#374151}
.adm-select{width:100%;padding:8px 12px;background:#111120;border:1px solid #1e1e2e;border-radius:8px;color:#e2e8f0;font-size:13px;outline:none;font-family:'Inter',sans-serif;-webkit-appearance:none;appearance:none}
.adm-select:focus{border-color:#7c3aed}
.adm-select option{background:#111120}
.adm-label{display:block;font-size:10px;font-weight:700;color:#4b5563;text-transform:uppercase;letter-spacing:.6px;margin-bottom:4px}
.adm-form-group{margin-bottom:14px}
.adm-grid2{display:grid;grid-template-columns:1fr 1fr;gap:14px}
.adm-modal-overlay{position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:999;display:flex;align-items:center;justify-content:center;padding:20px}
.adm-modal{background:#0d0d15;border:1px solid #2d2d3d;border-radius:16px;width:100%;max-width:500px;max-height:90vh;overflow-y:auto}
.adm-modal-header{padding:16px 20px;border-bottom:1px solid #1e1e2e;display:flex;align-items:center;justify-content:space-between}
.adm-modal-title{font-size:14px;font-weight:700;color:#fff}
.adm-modal-close{background:none;border:none;color:#6b7280;font-size:18px;cursor:pointer;line-height:1}
.adm-modal-body{padding:20px}
.adm-modal-footer{padding:14px 20px;border-top:1px solid #1e1e2e;display:flex;gap:8px;justify-content:flex-end}
.adm-log-item{padding:10px 0;border-bottom:1px solid #0f0f1a;display:flex;gap:12px;align-items:flex-start}
.adm-log-item:last-child{border-bottom:none}
.adm-log-dot{width:8px;height:8px;border-radius:50%;flex-shrink:0;margin-top:4px}
.adm-ann-item{padding:14px;background:#111120;border:1px solid #1e1e2e;border-radius:10px;margin-bottom:10px}
.adm-ann-title{font-size:13px;font-weight:700;color:#e2e8f0;margin-bottom:4px}
.adm-ann-body{font-size:12px;color:#9ca3af;line-height:1.6}
.adm-ann-meta{font-size:10px;color:#4b5563;margin-top:6px;display:flex;gap:12px}
.adm-progress-bar{height:6px;border-radius:3px;background:#1e1e2e;overflow:hidden;margin-top:6px}
.adm-progress-fill{height:100%;border-radius:3px;transition:width .6s}
.adm-section-header{display:flex;align-items:center;justify-content:space-between;margin-bottom:16px}
.adm-section-title{font-size:16px;font-weight:800;color:#fff;letter-spacing:-.3px}
.adm-section-sub{font-size:12px;color:#6b7280;margin-top:2px}
.adm-empty{text-align:center;padding:36px;color:#374151}
.adm-empty-icon{font-size:36px;opacity:.3;margin-bottom:10px}
.adm-search{display:flex;align-items:center;gap:8px;background:#111120;border:1px solid #1e1e2e;border-radius:8px;padding:7px 12px;flex:1;max-width:280px}
.adm-search input{background:none;border:none;outline:none;color:#e2e8f0;font-size:12px;width:100%;font-family:'Inter',sans-serif}
.adm-search input::placeholder{color:#374151}
.adm-login-wrap{min-height:100vh;background:#0a0a0f;display:flex;align-items:center;justify-content:center;font-family:'Inter',sans-serif}
.adm-login-box{width:100%;max-width:400px;background:#0d0d15;border:1px solid #1e1e2e;border-radius:18px;padding:40px 36px}
.adm-login-icon{width:52px;height:52px;background:linear-gradient(135deg,#7c3aed,#a855f7);border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:24px;margin:0 auto 16px}
.adm-login-title{text-align:center;font-size:22px;font-weight:800;color:#fff;margin-bottom:4px;letter-spacing:-.5px}
.adm-login-sub{text-align:center;font-size:12px;color:#6b7280;margin-bottom:28px}
.adm-login-error{background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.2);color:#ef4444;font-size:12px;padding:10px 13px;border-radius:8px;margin-bottom:16px}
.adm-tag{display:inline-flex;align-items:center;gap:4px;padding:2px 8px;border-radius:6px;font-size:10px;font-weight:600;background:#1a1a2e;color:#6b7280;border:1px solid #1e1e2e}
.adm-divider{height:1px;background:#1e1e2e;margin:16px 0}
.adm-notice{padding:12px 14px;border-radius:10px;font-size:12px;display:flex;gap:10px;align-items:flex-start;margin-bottom:14px}
.adm-notice-info{background:rgba(59,130,246,.08);border:1px solid rgba(59,130,246,.2);color:#93c5fd}
.adm-notice-warn{background:rgba(245,158,11,.08);border:1px solid rgba(245,158,11,.2);color:#fcd34d}
.adm-notice-danger{background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.2);color:#fca5a5}
`;
...
function AdminLoginPage({ onLogin }) {
  ...
  const handle = async () => {
    ...
    if (email === ADMIN_EMAIL && pw === ADMIN_PASSWORD) {
      onLogin({ uid: "admin", email: ADMIN_EMAIL, name: "System Admin", role: "admin" });
    } else {
  ...
  return (
    <div className="adm-login-wrap">
      ...
      <div className="adm-login-box">
        ...
        <div className="adm-login-title">Admin Console</div>
        <div className="adm-login-sub">MedRecord System Administration</div>
        {err && <div className="adm-login-error">⚠️ {err}</div>}
        <div className="adm-form-group">
          <label className="adm-label">Admin Email</label>
          <input className="adm-input" type="email" placeholder="admin@gmail.com" value={email} onChange={e => setEmail(e.target.value)} />
        </div>
        <div className="adm-form-group">
        ...
        <button className="adm-btn adm-btn-primary" style={{ width: "100%", justifyContent: "center", padding: "11px", fontSize: 13, marginTop: 6 }} onClick={handle} disabled={loading}>
...
function AdminDashboard({ patients, users, wardReports, logs, announcements }) {
  ...
  const reportsToday = wardReports.filter(r => r.date === new Date().toISOString().split("T")[0]).length;
  const wardCounts = {};
  ...
  const roleBreakdown = { nurse: 0, supervisor: 0, wardmaster: 0 };
  users.filter(u => !u.deleted).forEach(u => { if (roleBreakdown[u.role] !== undefined) roleBreakdown[u.role]++; });

  return (
    <div>
      <div className="adm-section-header">
        <div>
          <div className="adm-section-title">System Overview</div>
          <div className="adm-section-sub">Real-time snapshot of MedRecord EMR</div>
        </div>
        <div style={{ fontSize: 11, color: "#4b5563" }}>Last updated: {new Date().toLocaleTimeString()}</div>
      </div>

      <div className="adm-stat-grid">
        {[
          { icon: "🏥", val: activePatients, label: "Active Patients", color: "#10b981", delta: `${totalPatients} total` },
          { icon: "👥", val: totalUsers, label: "Registered Users", color: "#3b82f6", delta: `${roleBreakdown.nurse} nurses` },
          { icon: "📋", val: reportsToday, label: "Reports Today", color: "#a855f7", delta: `${wardReports.length} all time` },
          { icon: "🛡️", val: logs.length, label: "System Logs", color: "#f59e0b", delta: "audit trail" },
        ].map((s, i) => (
          <div key={i} className="adm-stat">
            ...
            <div className="adm-stat-delta" style={{ background: `${s.color}18`, color: s.color }}>{s.delta}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div className="adm-card">
          <div className="adm-card-header"><span className="adm-card-title">🗺️ Ward Occupancy (Active)</span></div>
          <div className="adm-card-body">
            {Object.keys(wardCounts).length === 0
              ? <div className="adm-empty"><div className="adm-empty-icon">🏥</div>No active patients</div>
              : Object.entries(wardCounts).sort((a,b) => b[1]-a[1]).map(([ward, count]) => (
                <div key={ward} style={{ marginBottom: 12 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}>
                    <span style={{ color: "#9ca3af" }}>{ward}</span>
                    <span style={{ color: "#e2e8f0", fontWeight: 700 }}>{count}</span>
                  </div>
                  <div className="adm-progress-bar">
                    <div className="adm-progress-fill" style={{ width: `${Math.min(100, (count / Math.max(activePatients, 1)) * 100)}%`, background: "linear-gradient(90deg,#7c3aed,#a855f7)" }} />
                  </div>
                </div>
              ))}
          </div>
        ...
        {/* Role breakdown */}
        <div className="adm-card">
          <div className="adm-card-header"><span className="adm-card-title">👥 Staff Role Breakdown</span></div>
          <div className="adm-card-body">
            {[
              { label: "Ward Nurses", val: roleBreakdown.nurse, color: "#3b82f6" },
              { label: "Supervisors", val: roleBreakdown.supervisor, color: "#10b981" },
              { label: "Ward Masters", val: roleBreakdown.wardmaster, color: "#a855f7" },
            ].map((r, i) => (
              <div key={i} style={{ marginBottom: 14 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}>
                  <span style={{ color: "#9ca3af" }}>{r.label}</span>
                  <span style={{ color: "#e2e8f0", fontWeight: 700 }}>{r.val} staff</span>
                </div>
                <div className="adm-progress-bar">
            ...
            <div style={{ fontSize: 12, color: "#6b7280", textAlign: "center" }}>Total: {totalUsers} registered staff</div>
          </div>
        </div>
      </div>
      ...
      <div className="adm-card">
        <div className="adm-card-header">
          <span className="adm-card-title">📢 Latest Announcements</span>
          <span className="adm-badge adm-badge-purple">{announcements.filter(a => !a.deleted).length} active</span>
        </div>
        <div className="adm-card-body">
          {announcements.filter(a => !a.deleted).slice(0, 3).length === 0
            ? <div className="adm-empty"><div className="adm-empty-icon">📢</div>No announcements yet</div>
            : announcements.filter(a => !a.deleted).slice(0, 3).map(a => (
              <div key={a.id} className="adm-ann-item">
                ...
                <div className="adm-ann-body">{a.body?.slice(0, 120)}{a.body?.length > 120 ? "…" : ""}</div>
                <div className="adm-ann-meta">
                  <span>{a.createdAt?.toDate ? a.createdAt.toDate().toLocaleDateString() : "—"}</span>
                  <span className={`adm-badge adm-badge-${a.priority === "urgent" ? "red" : a.priority === "important" ? "yellow" : "blue"}`}>{a.priority || "info"}</span>
                </div>
              </div>
            ))}
        </div>
      ...
      {/* Recent logs */}
      <div className="adm-card">
        <div className="adm-card-header"><span className="adm-card-title">🕐 Recent System Activity</span></div>
        <div className="adm-card-body">
          {logs.slice(0, 8).length === 0
            ? <div className="adm-empty"><div className="adm-empty-icon">📜</div>No logs yet</div>
            : logs.slice(0, 8).map((l, i) => (
              <div key={l.id || i} className="adm-log-item">
                <div className="adm-log-dot" style={{ background: l.action === "DELETE" ? "#ef4444" : l.action === "CREATE" ? "#10b981" : l.action === "UPDATE" ? "#f59e0b" : "#6b7280" }} />
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 12, color: "#e2e8f0", fontWeight: 600 }}>{l.action} · {l.detail}</div>
                  <div style={{ fontSize: 10, color: "#4b5563", marginTop: 2 }}>{l.adminEmail} · {l.timestamp?.toDate ? l.timestamp.toDate().toLocaleString() : "—"}</div>
                </div>
              </div>
            ))}
        </div>
...
function AdminUsers({ users, onRefresh, adminEmail, showToast }) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      await FB.saveSystemLog("CREATE", `New user: ${form.name} (${form.role})`, adminEmail);
      showToast("User created successfully.");
      setShowAdd(false);
      setForm({ name: "", email: "", password: "", role: "nurse", ward: "" });
      onRefresh();
    } catch (e) { showToast("Error: " + e.message, "error"); }
    setAdding(false);
  };
  ...
  const handleRoleChange = async (uid, role) => {
    try {
      await FB.updateUserRole(uid, role);
      await FB.saveSystemLog("UPDATE", `Role changed for UID ${uid} → ${role}`, adminEmail);
  ...
  const handleDelete = async (u) => {
    ...
    try {
      await FB.deleteUser(u.uid);
      await FB.saveSystemLog("DELETE", `User deleted: ${u.name} (${u.email})`, adminEmail);
      showToast("User deactivated.");
      onRefresh();
    } catch (e) { showToast("Error: " + e.message, "error"); }
  };

  const roleColor = { nurse: "blue", supervisor: "green", wardmaster: "purple" };
  const roleLabel = { nurse: "Ward Nurse", supervisor: "Supervisor", wardmaster: "Ward Master" };
    ...
    <div>
      <div className="adm-section-header">
        <div>
          ...
          <div className="adm-section-sub">{active.length} staff accounts</div>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <div className="adm-search"><span style={{ color: "#4b5563" }}>🔍</span><input placeholder="Search users…" value={search} onChange={e => setSearch(e.target.value)} /></div>
          <button className="adm-btn adm-btn-primary" onClick={() => setShowAdd(true)}>+ Add User</button>
        </div>
      </div>
      ...
      <div className="adm-card">
        <table className="adm-table">
          <thead>
            <tr>
              <th>Name</th><th>Email</th><th>Role</th><th>Ward</th><th>Joined</th><th>Actions</th>
            </tr>
          </thead>
          <tbody>
            {active.length === 0
              ? <tr><td colSpan={6} style={{ textAlign: "center", padding: 30, color: "#374151" }}>No users found</td></tr>
              : active.map(u => (
                <tr key={u.uid}>
                  <td>
                    <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
                      <div style={{ width: 28, height: 28, borderRadius: "50%", background: "linear-gradient(135deg,#7c3aed,#a855f7)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, color: "#fff", flexShrink: 0 }}>
                  ...
                  <td>{u.email}</td>
                  <td>
                    <select className="adm-select" style={{ width: "auto", padding: "3px 8px", fontSize: 11 }} value={u.role || "nurse"} onChange={e => handleRoleChange(u.uid, e.target.value)}>
                      <option value="nurse">Ward Nurse</option>
                      <option value="supervisor">Supervisor</option>
                      <option value="wardmaster">Ward Master</option>
                    </select>
                  </td>
                  <td>{u.ward || <span style={{ color: "#374151" }}>—</span>}</td>
                  <td>{u.createdAt?.toDate ? u.createdAt.toDate().toLocaleDateString() : "—"}</td>
                  <td>
                    <button className="adm-btn adm-btn-danger adm-btn-sm" onClick={() => handleDelete(u)}>🗑 Remove</button>
                  </td>
                </tr>
              ))}
          </tbody>
      ...
      {showAdd && (
        <div className="adm-modal-overlay" onMouseDown={e => { if (e.target === e.currentTarget) setShowAdd(false); }}>
          <div className="adm-modal">
            <div className="adm-modal-header">
              <span className="adm-modal-title">➕ Create New User</span>
            ...
            <div className="adm-modal-footer">
              ...
              <button className="adm-btn adm-btn-primary" onClick={handleAdd} disabled={adding}>{adding ? "Creating…" : "✚ Create User"}</button>
            </div>
          </div>
        </div>
      )}
...
// ─── ADMIN PATIENTS ────────────────────────────────────────────────────────────
function AdminPatients({ patients, adminEmail, showToast, onRefresh }) {
  const [search, setSearch] = useState("");
  ...
  const handleDelete = async (p) => {
    if (!window.confirm(`Permanently remove patient record for "${p.name}"?`)) return;
    try {
      await FB.deletePatient(p.id);
      await FB.saveSystemLog("DELETE", `Patient record removed: ${p.name} (EMR: ${p.emr})`, adminEmail);
    ...
    <div>
      <div className="adm-section-header">
        <div>
          ...
          <div className="adm-section-sub">{visible.length} records · read-only admin view</div>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          {["all","active","discharged"].map(f => (
            <button key={f} className="adm-btn adm-btn-ghost adm-btn-sm" style={filter===f?{borderColor:"#7c3aed",color:"#a855f7"}:{}} onClick={() => setFilter(f)}>{f.charAt(0).toUpperCase()+f.slice(1)}</button>
          ))}
          <div className="adm-search"><span style={{ color: "#4b5563" }}>🔍</span><input placeholder="Search patients…" value={search} onChange={e => setSearch(e.target.value)} /></div>
        </div>
      </div>
      ...
      <div className="adm-card">
        <table className="adm-table">
          <thead>
            <tr><th>Patient</th><th>EMR</th><th>Ward</th><th>Diagnosis</th><th>Status</th><th>Admitted</th><th>Actions</th></tr>
          </thead>
          <tbody>
            {visible.length === 0
              ? <tr><td colSpan={7} style={{ textAlign: "center", padding: 30, color: "#374151" }}>No records found</td></tr>
              : visible.map(p => (
                <tr key={p.id}>
                  <td><span style={{ color: "#e2e8f0", fontWeight: 600 }}>{p.name}</span></td>
                  <td><span style={{ fontFamily: "monospace", color: "#a855f7" }}>{p.emr || "—"}</span></td>
                  <td>{p.ward?.split("–")[0]?.trim() || "—"}</td>
                  <td>{p.diagnosis || <span style={{ color: "#374151" }}>—</span>}</td>
                  <td><span className={`adm-badge adm-badge-${p.status === "discharged" ? "yellow" : "green"}`}>{p.status || "active"}</span></td>
                  <td>{p.admission || "—"}</td>
                  <td><button className="adm-btn adm-btn-danger adm-btn-sm" onClick={() => handleDelete(p)}>🗑 Delete</button></td>
                </tr>
              ))}
          </tbody>
        </table>
...
// ─── ADMIN ANNOUNCEMENTS ──────────────────────────────────────────────────────
function AdminAnnouncements({ announcements, adminEmail, showToast }) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await FB.saveAnnouncement({ ...form, postedBy: adminEmail });
      await FB.saveSystemLog("CREATE", `Announcement: "${form.title}"`, adminEmail);
  ...
  const handleDelete = async (a) => {
    ...
    try {
      ...
      await FB.saveSystemLog("DELETE", `Announcement deleted: "${a.title}"`, adminEmail);
  ...
  return (
    <div>
      <div className="adm-section-title" style={{ marginBottom: 16 }}>📢 Announcements</div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        <div>
          <div className="adm-card">
            <div className="adm-card-header"><span className="adm-card-title">Post New Announcement</span></div>
            <div className="adm-card-body">
              ...
              <div className="adm-form-group"><label className="adm-label">Message *</label><textarea className="adm-input" placeholder="Write your announcement…" rows={4} style={{ resize: "vertical" }} value={form.body} onChange={e => setForm(f => ({ ...f, body: e.target.value }))} /></div>
              <div className="adm-grid2">
                ...
                <div className="adm-form-group"><label className="adm-label">Target Role</label>
              ...
              <button className="adm-btn adm-btn-primary" style={{ width: "100%", justifyContent: "center" }} onClick={handleSave} disabled={saving}>{saving ? "Posting…" : "📢 Post Announcement"}</button>
            </div>
          </div>
        </div>
        <div>
          <div className="adm-card-title" style={{ marginBottom: 12, color: "#e2e8f0" }}>Active Announcements ({active.length})</div>
          {active.length === 0
            ? <div className="adm-empty" style={{ background: "#0d0d15", borderRadius: 14, border: "1px solid #1e1e2e" }}><div className="adm-empty-icon">📢</div><div>No announcements</div></div>
            : active.map(a => (
              <div key={a.id} className="adm-ann-item">
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div className="adm-ann-title">{a.title}</div>
                  <button className="adm-btn adm-btn-danger adm-btn-sm" onClick={() => handleDelete(a)}>✕</button>
                </div>
                <div className="adm-ann-body">{a.body}</div>
                <div className="adm-ann-meta">
                  <span className={`adm-badge adm-badge-${a.priority === "urgent" ? "red" : a.priority === "important" ? "yellow" : "blue"}`}>{a.priority}</span>
                  <span className="adm-tag">→ {a.targetRole || "all"}</span>
                  <span>{a.createdAt?.toDate ? a.createdAt.toDate().toLocaleDateString() : "—"}</span>
                </div>
              </div>
            ))}
...
function AdminLogs({ logs }) {
  ...
  const visible = logs.filter(l => {
    const q = search.toLowerCase();
    const match = !q || l.detail?.toLowerCase().includes(q) || l.adminEmail?.toLowerCase().includes(q);
    if (filterAction !== "all") return match && l.action === filterAction;
    return match;
  });

  const actionColor = { DELETE: "#ef4444", CREATE: "#10b981", UPDATE: "#f59e0b", LOGIN: "#3b82f6" };

  return (
    <div>
      <div className="adm-section-header">
        <div>
          <div className="adm-section-title">System Audit Logs</div>
          <div className="adm-section-sub">{logs.length} total entries</div>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          {["all","CREATE","UPDATE","DELETE","LOGIN"].map(a => (
            <button key={a} className="adm-btn adm-btn-ghost adm-btn-sm" style={filterAction===a?{borderColor:"#7c3aed",color:"#a855f7"}:{}} onClick={() => setFilterAction(a)}>{a}</button>
          ))}
          <div className="adm-search"><span style={{ color: "#4b5563" }}>🔍</span><input placeholder="Search logs…" value={search} onChange={e => setSearch(e.target.value)} /></div>
        </div>
      </div>
      ...
      <div className="adm-card">
        <table className="adm-table">
          <thead>
            <tr><th>Action</th><th>Detail</th><th>Admin</th><th>Timestamp</th></tr>
          </thead>
          <tbody>
            {visible.length === 0
              ? <tr><td colSpan={4} style={{ textAlign: "center", padding: 30, color: "#374151" }}>No logs found</td></tr>
              : visible.map((l, i) => (
                <tr key={l.id || i}>
                  <td><span className="adm-badge" style={{ background: `${actionColor[l.action] || "#6b7280"}18`, color: actionColor[l.action] || "#6b7280", border: `1px solid ${actionColor[l.action] || "#6b7280"}30` }}>{l.action}</span></td>
                  <td style={{ color: "#e2e8f0" }}>{l.detail}</td>
                  <td>{l.adminEmail}</td>
                  <td>{l.timestamp?.toDate ? l.timestamp.toDate().toLocaleString() : "—"}</td>
                </tr>
              ))}
          </tbody>
        </table>
...
function AdminReports({ wardReports, archives, patients }) {
  const byDate = {};
  ...
  return (
    <div>
      <div className="adm-section-title" style={{ marginBottom: 16 }}>📊 Reports Overview</div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16, marginBottom: 20 }}>
        {[
          { icon: "📋", label: "Total Ward Reports", val: wardReports.length },
          { icon: "🗃️", label: "Archived Reports", val: archives.length },
          { icon: "📅", label: "Active Days", val: Object.keys(byDate).length },
        ].map((s, i) => (
          <div key={i} className="adm-stat">
            <div className="adm-stat-accent" style={{ background: "linear-gradient(90deg,#7c3aed,transparent)" }} />
            <div className="adm-stat-icon">{s.icon}</div>
            <div className="adm-stat-val">{s.val}</div>
            <div className="adm-stat-label">{s.label}</div>
          </div>
      ...
      <div className="adm-card">
        <div className="adm-card-header"><span className="adm-card-title">📅 Reports by Date</span></div>
        <table className="adm-table">
          <thead><tr><th>Date</th><th>Reports Submitted</th><th>Coverage</th></tr></thead>
          <tbody>
            {sortedDates.length === 0
              ? <tr><td colSpan={3} style={{ textAlign: "center", padding: 30, color: "#374151" }}>No reports yet</td></tr>
              : sortedDates.map(date => (
                <tr key={date}>
                  <td style={{ color: "#e2e8f0", fontWeight: 600 }}>{date}</td>
                  <td><span className="adm-badge adm-badge-purple">{byDate[date]} report{byDate[date] !== 1 ? "s" : ""}</span></td>
                  <td>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <div className="adm-progress-bar" style={{ flex: 1 }}>
                        <div className="adm-progress-fill" style={{ width: `${Math.min(100, (byDate[date] / WARDS.length) * 100)}%`, background: "linear-gradient(90deg,#7c3aed,#a855f7)" }} />
                      </div>
                      <span style={{ fontSize: 11, color: "#6b7280", whiteSpace: "nowrap" }}>{byDate[date]}/{WARDS.length} wards</span>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <div className="adm-card">
        <div className="adm-card-header"><span className="adm-card-title">🗃️ Recent Archived Reports</span></div>
        <table className="adm-table">
          <thead><tr><th>ID</th><th>Archived At</th><th>Submitted By</th></tr></thead>
          <tbody>
            {archives.slice(0, 10).length === 0
              ? <tr><td colSpan={3} style={{ textAlign: "center", padding: 30, color: "#374151" }}>No archives</td></tr>
              : archives.slice(0, 10).map((a, i) => (
                <tr key={a.id || i}>
                  <td><span style={{ fontFamily: "monospace", color: "#a855f7", fontSize: 11 }}>{a.id}</span></td>
                  <td>{a.archivedAt?.toDate ? a.archivedAt.toDate().toLocaleString() : "—"}</td>
                  <td>{a.submittedBy || "—"}</td>
                </tr>
              ))}
          </tbody>
...
// ─── ADMIN SETTINGS ────────────────────────────────────────────────────────────
function AdminSettings({ adminEmail, showToast }) {
  const [saved, setSaved] = useState(false);

  const handleClearLogs = async () => {
    if (!window.confirm("This will mark all system logs as archived. Continue?")) return;
    await FB.saveSystemLog("UPDATE", "Admin cleared/archived system logs", adminEmail);
    showToast("Logs archived.");
  };

  return (
    <div>
      <div className="adm-section-title" style={{ marginBottom: 16 }}>⚙️ System Settings</div>
      <div className="adm-notice adm-notice-warn">
        <span>⚠️</span>
        <span>Changes here affect the entire MedRecord system. Proceed with caution.</span>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        <div className="adm-card">
          <div className="adm-card-header"><span className="adm-card-title">🔐 Admin Credentials</span></div>
          <div className="adm-card-body">
            <div className="adm-notice adm-notice-info"><span>ℹ️</span><span>Admin credentials are hardcoded for security. Contact the developer to change them.</span></div>
            <div style={{ fontSize: 12, color: "#6b7280", lineHeight: 1.8 }}>
              <div>Email: <span style={{ color: "#a855f7", fontFamily: "monospace" }}>{ADMIN_EMAIL}</span></div>
              <div>Role: <span className="adm-badge adm-badge-purple">System Administrator</span></div>
            </div>
          </div>
        </div>
        <div className="adm-card">
          <div className="adm-card-header"><span className="adm-card-title">🗄️ Database</span></div>
          <div className="adm-card-body">
            <div style={{ fontSize: 12, color: "#6b7280", lineHeight: 2, marginBottom: 14 }}>
              <div>Provider: <span style={{ color: "#e2e8f0" }}>Firebase Firestore</span></div>
              <div>Project: <span style={{ color: "#e2e8f0", fontFamily: "monospace", fontSize: 11 }}>the-elites-nurses</span></div>
              <div>Auth: <span className="adm-badge adm-badge-green">Connected</span></div>
            </div>
            <button className="adm-btn adm-btn-ghost" onClick={handleClearLogs}>🗃️ Archive System Logs</button>
          </div>
        </div>
        <div className="adm-card">
          <div className="adm-card-header"><span className="adm-card-title">🏥 Ward Configuration</span></div>
          <div className="adm-card-body">
            <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 10 }}>Currently configured wards:</div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {WARDS.map(w => <span key={w} className="adm-tag">{w.split("–")[0].trim()}</span>)}
            </div>
            <div className="adm-notice adm-notice-info" style={{ marginTop: 14, marginBottom: 0 }}><span>ℹ️</span><span>Ward list is defined in the application code.</span></div>
          </div>
        </div>
        <div className="adm-card">
          <div className="adm-card-header"><span className="adm-card-title">📱 App Info</span></div>
          <div className="adm-card-body">
            <div style={{ fontSize: 12, color: "#6b7280", lineHeight: 2 }}>
              <div>App Name: <span style={{ color: "#e2e8f0" }}>MedRecord EMR</span></div>
              <div>Version: <span style={{ color: "#e2e8f0" }}>1.0.0</span></div>
              <div>AI Engine: <span style={{ color: "#a855f7" }}>Claude (Anthropic)</span></div>
              <div>Roles: <span style={{ color: "#e2e8f0" }}>Nurse · Supervisor · Ward Master · Admin</span></div>
            </div>
          </div>
        </div>
      </div>
...
function AdminApp({ onLogout }) {
  ...
  useEffect(() => {
    const unsubPt = FB.onPatients(setPatients);
    const unsubWR = FB.onWardReports(setWardReports);
    const unsubAR = FB.on24hrArchives(setArchives);
    const unsubLogs = FB.onSystemLogs(setLogs);
    const unsubAnn = FB.onAnnouncements(setAnnouncements);
    loadUsers();
    FB.saveSystemLog("LOGIN", "Admin logged in", ADMIN_EMAIL);
    return () => { unsubPt(); unsubWR(); unsubAR(); unsubLogs(); unsubAnn(); };
  }, []);

  const navItems = [
    { id: "dashboard", icon: "🏠", label: "Dashboard" },
    { id: "users", icon: "👥", label: "Users", badge: users.filter(u => !u.deleted).length },
    { id: "patients", icon: "🏥", label: "Patients", badge: patients.filter(p => !p.deleted && (p.status||"active")==="active").length },
    { id: "reports", icon: "📊", label: "Reports" },
    { id: "announcements", icon: "📢", label: "Announcements" },
    { id: "logs", icon: "🕐", label: "Audit Logs", badge: logs.length },
    { id: "settings", icon: "⚙️", label: "Settings" },
  ];

  const sectionTitles = { dashboard: "Dashboard", users: "User Management", patients: "Patient Records", reports: "Reports", announcements: "Announcements", logs: "Audit Logs", settings: "Settings" };
    ...
    <div className="admin-root">
      ...
      <nav className="adm-sidebar">
        <div className="adm-logo">
          <div className="adm-logo-badge">
            ...
            <div>
              ...
              <div className="adm-logo-sub">MedRecord System</div>
            </div>
          </div>
        </div>
        ...
        <div className="adm-nav">
          ...
          {navItems.map(n => (
            <button key={n.id} className={`adm-nav-btn ${section === n.id ? "active" : ""}`} onClick={() => setSection(n.id)}>
              ...
              {n.label}
              {n.badge !== undefined && <span className="adm-nav-badge">{n.badge}</span>}
            </button>
          ))}
          <div className="adm-nav-section">Session</div>
          <button className="adm-nav-btn" style={{ color: "#ef4444" }} onClick={() => { FB.saveSystemLog("LOGIN", "Admin logged out", ADMIN_EMAIL); onLogout(); }}>
            <span className="adm-ni">🚪</span>Logout
          </button>
        </div>
        ...
        <div className="adm-user-area">
          ...
          <div>
            <div className="adm-user-name">System Admin</div>
      ...
      <div className="adm-main">
        <div className="adm-topbar">
          <div>
            <div className="adm-topbar-title">{sectionTitles[section]}</div>
            <div className="adm-topbar-sub">MedRecord Admin Console · {new Date().toLocaleDateString()}</div>
          </div>
          <div className="adm-topbar-right">
            <span className="adm-badge adm-badge-green" style={{ fontSize: 10, padding: "3px 10px" }}>● Live</span>
            <span style={{ fontSize: 11, color: "#4b5563" }}>{ADMIN_EMAIL}</span>
          </div>
        </div>
        ...
        <div className="adm-content">
          ...
          {section === "users" && <AdminUsers users={users} onRefresh={loadUsers} adminEmail={ADMIN_EMAIL} showToast={showToast} />}
          {section === "patients" && <AdminPatients patients={patients} adminEmail={ADMIN_EMAIL} showToast={showToast} onRefresh={() => {}} />}
          {section === "reports" && <AdminReports wardReports={wardReports} archives={archives} patients={patients} />}
          {section === "announcements" && <AdminAnnouncements announcements={announcements} adminEmail={ADMIN_EMAIL} showToast={showToast} />}
          {section === "logs" && <AdminLogs logs={logs} />}
          {section === "settings" && <AdminSettings adminEmail={ADMIN_EMAIL} showToast={showToast} />}
        </div>
      </div>
    </div>
  );
  ...
  if (!user) return (
    <>
      ...
      <LoginPage onLogin={(u) => {
        ...
        if (u === "ADMIN") { setIsAdmin(true); return; }
        setUser(u);
      }} />
    </>
  );
